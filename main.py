    ...
    for account in accounts:
        ...
        for post in posts:
            for media in post['attachments']:
                mediaBundles = posts.data.get('accountMediaBundles', [])
                mediaIds = sum([x['accountMediaIds'] for x in mediaBundles if x['id'] == media['contentId']], [])
                accountMedia = posts.data.get('accountMedia', [])
                mediaIds.extend([x['id'] for x in accountMedia if x['id'] == media['contentId']])
